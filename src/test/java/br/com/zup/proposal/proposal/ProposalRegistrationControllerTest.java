...
class ProposalRegistrationControllerTest {
    ...
    public void test1 () throws Exception {
        ...
        var proposalResponse = new ProposalCreated(proposalRequest.toProposal());
    ...
    @Test
    @DisplayName("must read a proposal by id")
    @Transactional
    public void test3 () throws Exception {
        var proposal = new Proposal("43414120860" , "Foo Bar" , "foo@bar.com" , "Stree foo, 123" , BigDecimal.valueOf(2000d));
        var card = new Card("5409-8787-9540-5906");
        proposal.setCard(card);
        proposal.setStatus("SEM_RESTRICAO");
        manager.persist(proposal);

        var uri = UriComponentsBuilder.fromUriString("http://localhost:8080/api/proposal/3").build().toUri();

        var proposalResponse = new ProposalResponse(proposal);
        var response = mapper.writeValueAsString(proposalResponse);

        mockMvc.perform(get(uri)).andExpect(status().isOk()).andExpect(content().string(response));
    }
}